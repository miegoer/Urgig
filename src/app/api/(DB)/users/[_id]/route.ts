...
export async function PATCH(
  request: NextRequest,
  { params }: { params: { _id: string } } // missing _ for id
) {
  try {
    ...
    const user = await UserModel.findById(params._id).select('-password'); // missing _ for id

    // if doesn't exit -> return 404
    if (!user)
      return NextResponse.json(
        { error: 'user not found PATCH' },
        { status: 404 }
      );
    ...
    return NextResponse.json(
      { error: 'Failed to update user PATCH' },
      { status: 500 }
    );
  }
}
