...
export default function Upcoming() {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const events = await fetchAndTransformEvents(userId); // Fetch and transform events
        setArtistUpcomingEvents(events);
      } catch (error) {
  ...
  return (
    <div className="w-[320px] shadow-[0px_0px_0px_#272525] mt-5 rounded-[20px] p-5 bg-[#252531]">
      ...
      {artistUpcomingEvents.map((event: ArtistEvent) => {
          ...
          >
            ...
            <div className="flex flex-col text-center w-3/5">
              <span className={`uppercase mx-0 my-1.5 ${ubuntu.className}`}>
                <Link href={`/events/${event._id}`}>{event.name} too late</Link>
              </span>
